...
function FullScreenGraph() {
  ...
  const handleDataPointClick = (event, elements) => {
    ...
    if (oneDayReturnSymbol === "-") {
      comparisonText += ` and <span style="color:red;">Your portfolio went down by ${oneDayReturnChange.toFixed(2)}</span>.`;
    } else {
      comparisonText += ` and <span style="color:green;">Your portfolio went up by ${oneDayReturnChange.toFixed(2)}</span>.`;
    }
